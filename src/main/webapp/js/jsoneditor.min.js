...
// minLength, maxLength, and pattern
            "undefined"!=typeof this.schema.maxLength&&this.input.setAttribute("maxlength",this.schema.maxLength),"undefined"!=typeof this.schema.pattern?this.input.setAttribute("pattern",this.schema.pattern):"undefined"!=typeof this.schema.minLength&&this.input.setAttribute("pattern",".{"+this.schema.minLength+",}"),this.options.compact?this.container.classList.add("compact"):this.options.input_width&&(this.input.style.width=this.options.input_width),(this.schema.readOnly||this.schema.readonly||this.schema.template)&&(this.always_disabled=!0,this.input.setAttribute("readonly","true")),
// Set custom attributes on input element. Parameter is array of protected keys. Empty array if none.
            this.setInputAttributes(["maxlength","pattern","readonly","min","max","step"]),this.input.addEventListener("change",function(b){
...
// Compile and store the template
            this.schema.template?(this.template=this.jsoneditor.compileTemplate(this.schema.template,this.template_engine),this.refreshValue()):this.refreshValue()},postBuild:function(){
// Enable cleave.js support if library is loaded and config is available
        if(this._super(),window.Cleave&&this.schema.options&&"object"==typeof this.schema.options.cleave){new window.Cleave(this.input,this.schema.options.cleave)}},enable:function(){this.always_disabled||(this.input.disabled=!1,
...
// layout hasn't changed
            if(this.layout===JSON.stringify(h))return!1;for(this.layout=JSON.stringify(h),
// Layout the form
                                                                d=document.createElement("div"),a=0;a<h.length;a++){var k=this.theme.getGridRow();for(d.appendChild(k),b=0;b<h[a].editors.length;b++){var l=h[a].editors[b].key,m=this.editors[l];m.options.hidden?m.container.style.display="none":this.theme.setGridColumnSize(m.container,h[a].editors[b].width),k.appendChild(m.container)}}}else{if(d=document.createElement("div"),e){
...
//Pass the pane which holds the editor
                    "undefined"==typeof c.basicPane?
                        ...
                        c.addRow(d,o,e):c.addRow(d,o,c.basicPane)),e.id=c.getValidId(d.tab_text.textContent),
...
//Update the rowPane (same as self.rows[x].rowPane)
                    d.rowPane=e}});this.tabPanesContainer.firstChild;)this.tabPanesContainer.removeChild(this.tabPanesContainer.firstChild);
...
//"Borrow" from arrays code
        addTab:function(a){var b=this,c=b.rows[a].schema&&("object"===b.rows[a].schema.type||"array"===b.rows[a].schema.type);b.tabs_holder&&(b.rows[a].tab_text=document.createElement("span"),c?b.rows[a].tab_text.textContent=b.rows[a].getHeaderText():b.rows[a].tab_text.textContent="undefined"==typeof b.schema.basicCategoryTitle?"Basic":b.schema.basicCategoryTitle,b.rows[a].tab=b.theme.getTopTab(b.rows[a].tab_text,this.getValidId(b.rows[a].tab_text.textContent)),b.rows[a].tab.addEventListener("click",function(c){b.active_tab=b.rows[a].tab,b.refreshTabs(),c.preventDefault(),c.stopPropagation()}))},addRow:function(a,b,c){var d=this,e=this.rows.length,f="object"===a.schema.type||"array"===a.schema.type;
//Add a row
            d.rows[e]=a,
//rowPane stores the editor corresponding pane to set the display style when refreshing Tabs
                d.rows[e].rowPane=c,f?(d.addTab(e),d.theme.addTopTab(b,d.rows[e].tab)):
...
//so, when 'click' event is fired from a row, it gets the correct ("Basic") tab
                    d.rows[e].tab=d.rows[d.basicTab].tab,d.rows[e].tab_text=d.rows[d.basicTab].tab_text,d.rows[e].rowPane=d.rows[d.basicTab].rowPane)},
...
//If it's an orphan row (some property which has been deleted), return
            f.tab&&f.rowPane&&f.rowPane.parentNode&&(c&&f.tab==b.rows[b.basicTab].tab&&d||(a?f.tab_text.textContent=f.getHeaderText():(
...
// Overwrite existing file by default, leave other properties unchanged
        b.value[b.count][b.key]=a.target.result;else{var c={};
// Create empty object
...
// and the parent of the parent type has been set to 'array'
            a.schema.options&&a.schema.options.multiple&&1==a.schema.options.multiple&&a.parent&&"object"==a.parent.schema.type&&a.parent.parent&&"array"==a.parent.parent.schema.type&&this.uploader.setAttribute("multiple",""),this.uploader.addEventListener("change",function(b){if(b.preventDefault(),b.stopPropagation(),this.files&&this.files.length)
...
// Insert container at same position as input field
                    f.insertBefore(h,g),a=h):this.options.flatpickr.wrap=!1}this.flatpickr=window.flatpickr(a,this.options.flatpickr),this.options.flatpickr.inline===!0&&this.options.flatpickr.inlineHideInput===!0&&this.input.setAttribute("type","hidden")}},getValue:function(){if(!this.dependenciesFulfilled)return b;if("string"==this.schema.type)return this.value;if(""===this.value||this.value===b)return b;var a="time"==this.schema.format?"1970-01-01 "+this.value:this.value;return parseInt(new Date(a).getTime()/1e3)},setValue:function(a,b,c){if("string"==this.schema.type)this._super(a,b,c);else if(a>0){var d,e=new Date(1e3*a),f=e.getFullYear(),g=this.zeroPad(e.getMonth()+1),h=this.zeroPad(e.getDate()),i=this.zeroPad(e.getHours()),j=this.zeroPad(e.getMinutes()),k=this.zeroPad(e.getSeconds()),l=[f,g,h].join("-"),m=[i,j,k].join(":");d="date"==this.schema.format?l:"time"==this.schema.format?m:l+" "+m,this.input.value=d}},destroy:function(){this.flatpickr&&this.flatpickr.destroy(),this.flatpickr=null,this._super()},
// helper function
...
// Required to keep height
            var d=document.createElement("div");d.classList.add("signature-container");
// Create canvas for signature pad
            var e=document.createElement("canvas");e.setAttribute("name",b),e.classList.add("signature"),d.appendChild(e),a.signaturePad=new window.SignaturePad(e,{onEnd:function(){
...
// create button containers and add clear signature button
            var g=document.createElement("div"),h=document.createElement("button");h.classList.add("tiny","button"),h.innerHTML="Clear signature",g.appendChild(h),d.appendChild(g),this.options.compact&&this.container.setAttribute("class",this.container.getAttribute("class")+" compact"),(this.schema.readOnly||this.schema.readonly)&&(this.always_disabled=!0,f(this.inputs,function(a,b){e.setAttribute("readOnly","readOnly"),b.disabled=!0})),
...
// Icon only
     ...
     */
    setGridColumnSize:function(a,b){a.classList.add("col"),a.classList.add("s"+b)},/**
...
// Create and return button.
        var d=this._super(a,b,c);return d.classList.add("waves-effect","waves-light","btn"),d.style.fontSize="0.75rem",d.style.height="24px",d.style.lineHeight="24px",d.style.marginLeft="5px",d.style.padding="0 0.5rem",d},/**
...
// Color needs special attention.
     ...
     * @see https://github.com/Dogfalo/materialize/issues/2542#issuecomment-233458602
     */
    getTab:function(a){var b=document.createElement("li");return b.classList.add("tab"),b.style=b.style||{},this.applyStyles(b,{width:"100%",textAlign:"left",lineHeight:"24px",height:"24px",fontSize:"14px",cursor:"pointer"}),b.appendChild(a),b},/**
     * Marks specified tab as active.
     *
     * @returns {HTMLElement} The tab element.
     * @see https://github.com/Dogfalo/materialize/issues/2542#issuecomment-233458602
     */
    markTabActive:function(a){a.style=a.style||{},this.applyStyles(a,{width:"100%",textAlign:"left",lineHeight:"24px",height:"24px",fontSize:"14px",cursor:"pointer",color:"rgba(238,110,115,1)",transition:"border-color .5s ease",borderRight:"3px solid #424242"})},/**
     * Marks specified tab as inactive.
     *
     * @returns {HTMLElement} The tab element.
     * @see https://github.com/Dogfalo/materialize/issues/2542#issuecomment-233458602
     */
    markTabInactive:function(a){a.style=a.style||{},this.applyStyles(a,{width:"100%",textAlign:"left",lineHeight:"24px",height:"24px",fontSize:"14px",cursor:"pointer",color:"rgba(238,110,115,0.7)"})},/**
...
// Append an error message div.
            c=document.createElement("div"),c.classList.add("error-text","red-text"),c.textContent=b,d.appendChild(c))},/**
...
// Remove all elements having class .error-text.
     ...
     */
    getSelectInput:function(a){var b=this._super(a);return b.classList.add("browser-default"),b},/**
     ...
     */
    getModal:function(){var a=document.createElement("div");return a.classList.add("card-panel","z-depth-3"),a.style.padding="5px",a.style.position="absolute",a.style.zIndex="10",a.style.display="none",a},/**
...
// @see http://materializecss.com/icons.html
        var c=document.createElement("i");c.classList.add(this.icon_class);var d=document.createTextNode(b);return c.appendChild(d),c}}),h.defaults.templates["default"]=function(){return{compile:function(a){var b=a.match(/{{\s*([a-zA-Z0-9\-_ \.]+)\s*}}/g),c=b&&b.length;
